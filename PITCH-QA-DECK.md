...
2. **"Shell 23/23" is not in this repo.** `README.md` records `npm run e2e:shell` at
   **14 / 14**, and the read-only proxy self-test separately at 14 / 14. The other suite
   numbers check out: 42 Daml scripts, e2e 28/28, actions 18/18, best-exec 8/8, hosted
   87/87 across three browsers, MCP 25/25.
3. **Slide 10 undersells what is verified.** The tile says 42 Daml scripts; the full
...
**Say —** Two DevNet participants running the same package including HackCanton's own node, the settled trades and five atomic baskets on that tile, sixteen best-execution attestations, forty-two Daml scripts green, and a hosted read-only desk over live ledger state.
**Katakan —** Dua participant DevNet menjalankan package yang sama termasuk node milik HackCanton sendiri, settled trade dan lima basket atomik di tile itu, enam belas attestation best execution, empat puluh dua Daml script hijau, dan hosted desk read-only di atas state ledger yang hidup.
