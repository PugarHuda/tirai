...
no collection risk. This one is built: `venue` and `feeBps` on the RFQ, the cut
split off inside `SettleQuote`, and the amount recorded on the trade report so
the audit trail states it. One settlement on DevNet cleared 4,250,000 at 25 bps —
10,625 to the venue, 4,239,375 to the dealer. Plus CIP-0047 featured-app markers,
which are NOT built: no marker in the code.

**Two things not to do.** Do not state a bps number as *the* rate — the mechanism
runs, the rate is unset, and blank charges nothing. If pressed, 25 bps is the
number in the DevNet settlement, not a price list. Do not invent a market size;
there is no TAM figure in this repo. If a judge presses, give the shape: fee on
notional, institutional ticket sizes, sized with a design partner.

**One limit to volunteer if asked.** On the registry rail — Canton Coin, CBTC —
the fee is not taken. That cash moves through the issuer's allocation, not
through a holding this desk can split, so the report reads no fee there. Say it
plainly; it is a real edge and pretending otherwise is worse than the gap.

The footer line on the slide — zero revenue, no paying customer — is there so you
can say it before anyone asks.
...
Two DevNet participants running the same package, including HackCanton's own node.
Forty-seven settled trades and five atomic baskets, sixteen attestations,
forty-four Daml scripts green, hosted read-only desk over live state.
